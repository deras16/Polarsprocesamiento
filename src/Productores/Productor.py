...
class Productor(Model):
    ...
    def extract(self) -> pl.DataFrame:
        query_productores = """
           select folio idfolio , departamento, municipio, tipo_prod, geo_est ->> 'Altitude' as Altitude, 
                geo_est ->> 'Latitude' as Latitude, geo_est ->> 'Longitude' as Longitude
                from "hq_dea_3a9df112-2351-459e-97a6-468d1cfaaf91"."EXPGB_2$1"
        """
    ...
    def transfomation_validations(self, df: pl.DataFrame) -> pl.DataFrame:
        df = df.rename({"idfolio":"IdFolio","departamento": "Departamento", "municipio": "Municipio", "tipo_prod": "TipologiaProd", "altitude": "Altitud", "latitude": "Latitud", "longitude": "Longitud"})
        df = df.with_columns(
